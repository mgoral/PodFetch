...
impl Session{
    ...
    pub fn insert_session(&self, conn: &mut AnyConnection) -> Result<Self, diesel::result::Error>{
        diesel::insert_into(sessions::table)
            .values(self)
            .get_result(conn)
    }

    pub fn cleanup_sessions(conn: &mut AnyConnection) -> Result<usize, diesel::result::Error>{
        diesel::delete(sessions::table.
            filter(sessions::expires.lt(Utc::now().naive_utc())))
            .execute(conn)
    }

    pub fn find_by_session_id(session_id: &str, conn: &mut AnyConnection) -> Result<Self, diesel::result::Error>{
        sessions::table
